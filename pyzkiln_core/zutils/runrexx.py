...
def runrexx(authorized, library, program_info, inputdata, outputinfo):
    ...
    Returns:
         Returns a structure that contains data with the following keys:
            ...
            systs_prt_file: The filename that the SYSTSPRT DD pointed to in case any
                   data is there.
            sys_prt_file: The filename that the SYSPRINT DD pointed to in case any
                   data is there
    """
    ...
    systs_in_file = f"{cwd}/systsin.{static_time}"
    # This will hold any MVS messages
    sys_prt_file = f"{cwd}/sysprt.{static_time}"
    # This will hold TSO output
    systs_prt_file = f"{cwd}/systsprt.{static_time}"
    ...
    # Take the input data and put it into a file.
    _write_out_the_input(inputdata, systs_in_file)

    # Now create a DD that points to the created file
    dd_list.append(DDStatement("SYSTSIN", FileDefinition(systs_in_file)))
    ...
    dd_list.append(DDStatement("SYSPRINT", FileDefinition(sys_prt_file)))
    dd_list.append(DDStatement("SYSTSPRT", FileDefinition(systs_prt_file)))
    ...
    if return_code_dict["rc"] == 0:
        os.remove(systs_in_file)

    # Return all data to the caller in a dictionary
    return_data = {
        "returninfo": return_code_dict,
        "systsprtfile": systs_prt_file,
        "sysprintfile": sys_prt_file,
    }
...
def main():
    ...
    To run this sample make sure the member name HELOWRLD.
